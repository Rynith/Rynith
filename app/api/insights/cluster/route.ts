...
export async function POST(req: Request) {
  ...
  // Collect recent embedding rows with sentiment/topics
  const q = sb
  ...
  // Re-shape
  const rows: EmbRow[] = (rowsRaw || []).map((r: any) => ({
    id: r.review_id,
    org_id: r.org_id,
  ...
  for (const r of rows) {
    if (!r.org_id || !Array.isArray(r.embedding)) continue;
  ...
  for (const [org_id, arr] of byOrg.entries()) {
    ...
    // Greedy leader clustering with cosine threshold
    const THRESH = 0.88;
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      for (let c = 0; c < clusters.length; c++) {
        const sim = dot(p, clusters[c].centroid); // centroid is kept normalized
        if (sim > bestSim) {
          bestSim = sim;
          best = c;
        }
      ...
      if (bestSim >= THRESH && best >= 0) {
        ...
        // update centroid (simple average then renorm)
        const all = clusters[best].ids.map((idx) => points[idx]);
        const m = new Array(points[0].length).fill(0);
    ...
    const stats = clusters
      .map((c, ci) => {
        ...
        // centroid as float8[] (already normalized number[])
        return {
          org_id,
          period_start,
          period_end,
      ...
      .filter((s) => s.size >= 2) // keep clusters with at least 2 reviews
      .sort((a, b) => b.size - a.size)
      .slice(0, 8);

    // Upsert: easiest is to clear existing window for org, then insert fresh
    const sb = supabaseService();
    await sb
      .from("topic_clusters")
      .delete()
